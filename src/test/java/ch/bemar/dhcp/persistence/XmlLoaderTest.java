...
public class XmlLoaderTest {
	...
	void testReadConfig() throws JAXBException, IOException {
		...
		Assertions.assertEquals("jdbc:h2:mem:testdb1;DB_CLOSE_DELAY=-1", config.getConnection().getPropertyValueByName(Configuration.PROP_CON_URL));
		Assertions.assertEquals("org.h2.Driver", config.getConnection().getPropertyValueByName(Configuration.PROP_DRIVER_CLASS));
		...
		Assertions.assertTrue(
				config.getConnection().containsClassname("ch.bemar.dhcp.persistence.DbLease"));
		Assertions.assertTrue(
				config.getConnection().containsClassname("ch.bemar.dhcp.persistence.Dingens"));
