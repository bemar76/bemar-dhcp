...
class IPRangeCalculatorTest {
	...
	void calculateTest_Cidr22() throws UnknownHostException {
		...
		System.out.println("Broadcast Adress: " + IPRangeCalculator.getBroadcastAddress(startIP, subnetMask));
		Assertions.assertEquals("/192.168.3.255", IPRangeCalculator.getBroadcastAddress(startIP, subnetMask).toString());
		try {
			List<String> allIPsInRange = IPRangeCalculator.calculateAllIPsInRange(startIP, endIP, subnetMask);
	...
	void calculateTest_Cidr24() throws UnknownHostException {
		...
		System.out.println("Broadcast Adress: " + IPRangeCalculator.getBroadcastAddress(startIP, subnetMask));
		Assertions.assertEquals("/192.168.0.255", IPRangeCalculator.getBroadcastAddress(startIP, subnetMask).toString());
		try {
			List<String> allIPsInRange = IPRangeCalculator.calculateAllIPsInRange(startIP, endIP, subnetMask);
			System.out.println("All IPs within the range:");
			for (String ip : allIPsInRange) {
	...
	void calculateTest_Cidr17() throws UnknownHostException {
		...
		System.out.println("Broadcast Adress: " + IPRangeCalculator.getBroadcastAddress(startIP, subnetMask));
		Assertions.assertEquals("/192.168.127.255", IPRangeCalculator.getBroadcastAddress(startIP, subnetMask).toString());
		try {
			List<String> allIPsInRange = IPRangeCalculator.calculateAllIPsInRange(startIP, endIP, subnetMask);
			System.out.println("All IPs within the range:");
			for (String ip : allIPsInRange) {
